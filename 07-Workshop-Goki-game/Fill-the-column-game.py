...
# place player marker - check if the column is full
...
def is_horizontal(ma, row, col, player_num, slots_count):
    ...
    4
    3
    If we do not check for both there will be winner, but we can not know that.
...
def is_winner(ma, row, col, player_num, slots_count=4):
    ...
    if any([is_horizontal(ma, row, col, player_num, slots_count),
        ...
        is_left_diagonal(ma, row, col, player_num, slots_count), is_down, ]):
        return True
    return False
